...
export const strengthColor: NumbColorFunc = (count, dict) => {
  if (count < 2) return { label: `${dict.poor}`, color: 'error.main' };
  if (count < 3) return { label: `${dict.weak}`, color: 'warning.main' };
  if (count < 4) return { label: `${dict.normal}`, color: 'warning.dark' };
  if (count < 5) return { label: `${dict.good}`, color: 'success.main' };
  if (count < 6) return { label: `${dict.strong}`, color: 'success.dark' };
